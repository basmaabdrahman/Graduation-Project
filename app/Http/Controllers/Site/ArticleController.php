...
class ArticleController extends Controller
{
    public function index()
    {
        ...
        $articles = DB::table('articles')->where("user_type","=","1")->
            where("cate_id", $userCategory)->latest('created_at','desc')->take(4)->get();

        return view('site.user.adult.articles.index0',compact('articles'));
    }
    public function search(){
        $search_text=$_GET['query'];
        $article=Article::where('desc','LIKE','%'.$search_text.'%')->get();
        return view('site.user.adult.articles.search',compact('article'));
    }
